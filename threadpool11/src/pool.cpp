...
Pool::Pool(std::size_t worker_count)
    : worker_count_(0), active_worker_count_(0), are_all_really_finished_{true},
...
void Pool::waitAll()
{
    std::unique_lock<std::mutex> notify_all_finished_lock(
        notify_all_finished_mutex_);

    notify_all_finished_signal_.wait(notify_all_finished_lock, [this]() {
        return are_all_really_finished_.load();
    });
}
...
{
    return worker_count_.load() - active_worker_count_.load();
}
...
void Pool::spawnWorkers(std::size_t n)
{
    ...
    while (n-- > 0) {
        new Worker(*this); //! Worker class takes care of its de-allocation
                           //! itself after here
        ++worker_count_;
    }
}
