...
class dense_bitset
{
  public:
    ...
    */
    ...
    inline void resize(size_t n)
    {
        ...
        // 位集合需要的位块数   sizeof(size_t) * 8 = 位块的位数
        // 位块数=位集合的大小除以每个位块的位数+是否有多余的位数(多余的位数为1个位块)
        arrlen = (n / (sizeof(size_t) * 8)) + (n % (sizeof(size_t) * 8) > 0);
        // 重新分配内部数组的内存空间=位块数 * 每个位块的大小
    ...
    */
    inline bool set_bit_unsync(size_t b)
    {
        ...
        bit_to_pos(b, arrpos, bitpos);
        const size_t mask(size_t(1) << size_t(bitpos));
        bool ret = array[arrpos] & mask; // 在bitpos处具有唯一位‘1’，其余位为0
        array[arrpos] |= mask; // 设置在'bitpos'位的值 = 1
        return ret;
    }
  ...
  private:
    inline static void bit_to_pos(size_t b, size_t &arrpos, size_t &bitpos)
    {
        ...
        arrpos = b / (8 * sizeof(size_t)); // 数组位置的‘arrpos’索引
